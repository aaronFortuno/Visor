...
import { controlSession, deleteSession as apiDeleteSession, fetchEvents, renameSession } from "../lib/api";
...
export function SessionView({
  ...
  useEffect(() => {
    // Only init terminal when in terminal mode; skip for Ollama sessions
    if (isOllama || viewMode !== "terminal" || !containerRef.current) return;
  ...
  useEffect(() => {
    if (isOllama) return; // No PTY output for Ollama sessions
  ...
  useEffect(() => {
    ...
    if (viewMode === "terminal") {
      fetchEvents(session.id, { limit: 500 }).then(({ events }) => {
