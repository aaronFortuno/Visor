...
import { validateAndSetToken } from "../lib/api";
...
export function LoginScreen() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setAuthError("");
    setValidating(true);
    const ok = await validateAndSetToken(trimmed);
    setValidating(false);

    if (!ok) {
      setAuthError("Invalid token or server unreachable");
    }
  };
      ...
      <div className="w-full max-w-sm md:max-w-md">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <div className="relative">
              <input
                ...
                value={token}
                onChange={(e) => { setTokenValue(e.target.value); setAuthError(""); }}
          ...
          <button
            ...
            onClick={handlePaste}
            disabled={validating}
            className="w-full py-3 border border-dashed border-visor-border rounded-lg text-gray-400 hover:text-white hover:border-visor-accent transition-colors flex items-center justify-center gap-2 disabled:opacity-30"
          >
          ...
          {authError && (
            <p className="text-visor-red text-sm text-center">{authError}</p>
          )}
